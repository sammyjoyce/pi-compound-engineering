...
export function registerQuestionTool(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		renderCall(args, theme) {
			let text = theme.fg("toolTitle", theme.bold("compound_question ")) + theme.fg("muted", args.question);
		...
		renderResult(result, _options, theme) {
			...
			if (details.wasCustom) return new Text(theme.fg("success", "✓ ") + theme.fg("muted", "(wrote) ") + theme.fg("accent", details.answer), 0, 0);
