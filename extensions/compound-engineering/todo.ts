...
export function registerTodoTool(pi: ExtensionAPI): void {
	...
	const reconstructState = (ctx: ExtensionContext) => {
		...
		for (const entry of ctx.sessionManager.getBranch()) {
			...
			if (msg.role !== "toolResult" || msg.toolName !== "compound_todo") continue;
	...
	pi.registerTool({
		...
		renderCall(args, theme) {
			let text = theme.fg("toolTitle", theme.bold("compound_todo ")) + theme.fg("muted", args.action);
	...
	pi.registerCommand("compound_todos", {
		description: "Show all compound_todo items on the current branch",
